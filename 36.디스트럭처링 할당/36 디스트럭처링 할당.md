...
**배열 디스트럭처링 할당의 대상(할당문의 우변)은 이터러블이어야 하며, 할당 기준은 배열의 인덱스**다. 즉, 순서대로 할당한다.
...
```jsx
const user = { firstName: "Jaehong", lastName: "Jeong" };
...
console.log(firstName, lastName); // Jaehong Jeong
```
...
console.log(fn, ln); // Jaehong Jeong
```

객체 디스트럭처링 할당을 위한 변수에 기본값을 설정할 수 있다.

```jsx
const { firstName = "Jaehong", lastName } = { lastName: "Jeong" };
console.log(firstName, lastName); // Jaehong Jeong

const { firstName: fn = "Jaehong", lastName: ln } = { lastName: "Jeong" };
console.log(fn, ln); // Jaehong Jeong
```

객체 디스트러처링 할당은 객체에서 프로퍼티 키로 필요한 프로퍼티 값만 출출하여 변수에 할당하고 싶을 때 유용하다.
